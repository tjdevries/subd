...
pub async fn find_or_create_and_save_score(
    ...
    user_id: Uuid,
    score: BigDecimal,
) -> Result<ai_songs_vote::Model> {
    Ok(sqlx::query_as!(
        ai_songs_vote::Model,
        r#"
                INSERT INTO ai_songs_vote (song_id, user_id, score, good_song)
                VALUES ($1, $2, $3, $4)
...
impl ai_songs_vote::Model {
    #[allow(dead_code)]
    pub async fn save(&self, pool: &PgPool) -> Result<Self> {
        Ok(sqlx::query_as!(
            Self,
            r#"
                INSERT INTO ai_songs_vote
    ...
    pub async fn find_or_create_and_save_good_song(
        ...
        Ok(sqlx::query_as!(
            Self,
            r#"
            INSERT INTO ai_songs_vote (song_id, user_id, good_song)
            VALUES ($1, $2, $3)
            ON CONFLICT (song_id, user_id)
    ...
    pub async fn update_score_for_current_song(
        pool: &PgPool,
        user_id: Uuid,
        score: BigDecimal,
    ) -> Result<Self> {
