...
pub struct OldOBSMessageHandler {
...
#[async_trait]
impl EventHandler for OldOBSMessageHandler {
    async fn handle(
        self: Box<Self>,
...
pub async fn handle_obs_commands<C: twitch_api::HttpClient>(
    ...
    let _ = match command {
        ...
        // !filter 3D-Transform-Perspective
        "!filter" => {
            ...
            let filter_details =
                match obs_client.filters().get("begin", filter).await {
                    Ok(val) => Ok(val),
                    Err(err) => Err(err),
                }
                .map_err(|e| e.to_string())?;
