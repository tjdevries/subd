...
// I wish we could just define this somewhere else
pub async fn handle_obs_commands(
    ...
    match splitmsg[0].as_str() {
        ...
        "!grow" | "!scale" => {
            ...
            trigger_grow(source, &base_scale, x, y, &obs_client).await
        }
        ...
        "!hide" => hide_sources(MEME_SCENE, &obs_client).await,
        "!show" => set_enabled(MEME_SCENE, source, true, &obs_client).await,
        "!def_ortho" => default_ortho(source, duration, &obs_client).await,
        "!ortho" => {
            if splitmsg.len() < 3 {
                return Ok(());
            };
        ...
        "!tr" => top_right(source, &obs_client).await,

        "!bl" => bottom_right(source, &obs_client).await,

        // ================ //
        // Compound Effects //
        // ================ //
        "!norm" => norm(&source, &obs_client).await,
        ...
        "!follow" => {
            ...
            follow(source, scene, leader, &obs_client).await
        }
        "!staff" => staff(DEFAULT_SOURCE, &obs_client).await,
        ...
        "!create_filters_for_source" => {
            create_filters_for_source(source, &obs_client).await
        }
        ...
        // TODO: Take in Scene
        "!source" => {
            print_source_info(source, DEFAULT_SCENE, &obs_client).await
        }

        "!outline" => {
            let source = splitmsg[1].as_str();
            outline(source, &obs_client).await
        }
        ...
        // ====================== //
        "!memes" => {
            set_enabled(DEFAULT_SCENE, MEME_SCENE, true, &obs_client).await
        }

        "!nomemes" | "!nojokes" | "!work" => {
            set_enabled(DEFAULT_SCENE, MEME_SCENE, false, &obs_client).await
        }
        ...
        "!chat" => trigger_hotkey("OBS_KEY_L", &obs_client).await,

        "!code" => trigger_hotkey("OBS_KEY_H", &obs_client).await,
